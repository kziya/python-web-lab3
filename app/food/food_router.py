...
async def get_food_by_id(food_id: str, db = Depends(get_db)):
    return food_crud.get_food_by_id(db, food_id)


@router.post("/food", response_model=food_schemas.Food)
async def create_food(createFood: food_schemas.CreateFood, db = Depends(get_db)):
...
async def update_food(food_id: str, updateFood: food_schemas.UpdateFood, db = Depends(get_db)):
    return food_crud.updated_food(db, food_id, updateFood.name, updateFood.price)


@router.delete("/food/{food_id}", response_model=None)
async def delete_food(food_id: str, db = Depends(get_db)):
